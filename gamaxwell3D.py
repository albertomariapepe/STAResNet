...
if resume != -1:
    ...
    model.load_weights('models/GA_ResNet_epoch_'+str(resume) + '.h5')
    print("Done!")
...
for epoch in range(resume+1,epochs):
    ...
    if avg_val_loss/count < max_val_loss:
        ...
        model.save_weights("models/GA_ResNet_epoch_"+str(epoch)+".h5")
    ...
    np.save('models/GA_training_losses_'+ str(resume)+'.npy', np.array(training_losses))
    np.save('models/GA_validation_losses_'+ str(resume)+'.npy', np.array(validation_losses))
