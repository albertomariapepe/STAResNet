from networks import CliffordResNet, STAResNet, CliffordResNet2D
import os
...
for epoch in range(resume+1, epochs):
    ...
    if avg_val_loss/count < max_val_loss:
        ...
        model.save_weights("drive/MyDrive/maxwell/GA_ResNet_epoch_"+str(epoch)+".h5")
    ...
    np.save('drive/MyDrive/maxwell/GA_training_losses_'+ str(resume)+'.npy', np.array(training_losses))
    np.save('drive/MyDrive/maxwell/GA_validation_losses_'+ str(resume)+'.npy', np.array(validation_losses))
...
plt.savefig('drive/MyDrive/maxwell/GAA_losses_plot.png')
